export class HeroAnimation {
    ...
    createParticles() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            this.particles.push({
                ...
                type: isTag ? 'tag' : 'dot',
                tag: isTag ? tags[Math.floor(Math.random() * tags.length)] : null,
    ...
    drawGrid() {
        ...
        const ctx = this.ctx;
        const spacing = 80;
        ctx.strokeStyle = 'rgba(var(--color-accent-rgb, 45, 45, 45), 0.03)';
        ctx.lineWidth = 0.5;
    ...
    drawBrackets() {
        if (!this.ctx) return;
        const ctx = this.ctx;
        const accent = getComputedStyle(document.documentElement).getPropertyValue('--color-accent').trim() || '#2D2D2D';
    ...
    animateContent() {
        const selectors = ['.hero-eyebrow', '.hero-title', '.hero-subtitle', '.hero-actions'];
        const elements = [];
